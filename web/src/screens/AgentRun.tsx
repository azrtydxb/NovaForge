...
/** Tool cursors resume durable history; best-effort state events never reset
 * that cursor. A pending tool event is not evidence the call completed. */
export function AgentRun() {
  ...
  const replay = useRef({ base: "", cursor: "" });
  useEffect(() => {
    if (replay.current.base !== base) replay.current = { base, cursor: "" };
    if (!live) return;
    ...
    void api
      .events(
        ...
        controller.signal,
        (event, data, cursor) => {
          if (cursor) replay.current.cursor = cursor;
          if (event === "error")
            throw new Error(
