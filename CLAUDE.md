...
- **Organizations are a hard security boundary.** Every query carries an org predicate
  ...
  another org and be believed. The exceptions (`work.Store.OpenEpics`,
  `agents.Store.staleRunning`, git-platform's `ListOrganizationsWithRepositories`) are
  platform-worker queries that return only ids and re-enter each org's scope before reading
  anything; each says so in its doc comment. The RPC admits only a platform token
  (`svcauth.MintPlatform`, a scope with `IsPlatformWorker()` and no org), which every
  org-scoped path refuses.
