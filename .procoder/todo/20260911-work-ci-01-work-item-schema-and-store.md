...
- [ ] Implement `Create` allocating `seq` inside the insert with `COALESCE((SELECT MAX(seq) FROM work_items WHERE org_id=$1), 0) + 1` in a single statement so concurrent creates cannot collide, and deriving `key` as `'NF-' || seq`.
- [ ] Implement every read with an explicit `org_id = $1` predicate taken from `authz.FromContext`, never from an argument supplied by the caller's request body.
