...
// RecordBoundary is used for every workspace, even when no stdio server opens.
// The caller records a durable pre-create obligation before provisioning.
func (p *Provisioner) RecordBoundary(ctx context.Context, runID uuid.UUID) error {
	if p.recordCleanup == nil {
		return fmt.Errorf("workspace recorder unavailable")
	}
	id, err := p.stdioIdentity(ctx, runID)
	if err != nil {
		return err
	}
	return p.recordCleanup(ctx, id)
}

// RecoveryIdentity discovers only this exact run's tagged namespace and pod.
// It is not termination evidence; missing resources and UID conflicts stay pending.
func (p *Provisioner) RecoveryIdentity(ctx context.Context, runID uuid.UUID) (Identity, error) {
	return p.stdioIdentity(ctx, runID)
}
