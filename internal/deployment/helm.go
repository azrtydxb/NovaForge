...
func (h *HelmExecutor) run(ctx context.Context, op Operation, observe bool) (Result, error) {
	...
	if apierrors.IsNotFound(err) {
		if !coversExecution(credentialDeadline(op)) {
			if err := h.revoke(ctx, op); err != nil {
				return Result{}, ErrUncertain
			}
			return Result{}, errors.New("deployment authority does not cover the execution window")
		}
		...
		if cred.SecretName != CredentialSecretName(op) || len(validation.IsDNS1123Subdomain(cred.SecretName)) > 0 || !coversExecution(cred.ExpiresAt) || cred.ExpiresAt.After(credentialDeadline(op)) {
			revokeErr := h.revoke(ctx, op)
			if revokeErr != nil {
