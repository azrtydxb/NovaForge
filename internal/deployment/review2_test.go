...
func janitorContext(ctx context.Context) context.Context {
	...
	scope.ServiceName = "deployment-cleanup"
...
func (c *journalCredentials) Prepare(ctx context.Context, op Operation, _ string, _ time.Duration) (Credential, error) {
	...
	return Credential{SecretName: CredentialSecretName(op), ExpiresAt: credentialDeadline(op)}, c.prepareErr
}
...
func TestReviewCredentialCleanupConfirmationIsExactAndFenced(t *testing.T) {
	...
	otherService := scope
	otherService.ServiceName = "ci-credentials"
	...
	for _, tc := range []struct {
		...
		{"other service", authz.WithScope(context.Background(), otherService), 1, h.Revision()},
	} {
		t.Run(tc.name, func(t *testing.T) {
