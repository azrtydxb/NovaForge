...
// cleaned" operation. Only the named cleanup service may call it. The trusted
...
func (s *Service) CleanupCredentials(ctx context.Context, id uuid.UUID, attempt int, providerBinding string) error {
	scope, err := authz.FromContext(ctx)
	if err != nil || scope.OrgID == uuid.Nil || scope.ActorKind != "service" || scope.ServiceName != "deployment-cleanup" || scope.ActorID != uuid.Nil {
		return errors.New("credential cleanup requires the organization deployment-cleanup service")
	}
	op, err := s.Get(ctx, id)
	if err != nil {
		return err
