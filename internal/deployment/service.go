...
func (s *Service) authorize(ctx context.Context, req Request) (authz.Scope, *Target, error) {
	scope, err := authz.FromContext(ctx)
	if err != nil {
		return scope, nil, err
	}
	if scope.OrgID == uuid.Nil || scope.ActorID == uuid.Nil || (scope.ActorKind != "agent" && scope.ActorKind != "user") {
		return scope, nil, errors.New("deployment requires an organization actor")
	}
	if scope.ActorKind == "user" && scope.Role != "member" && !scope.IsOrgAdmin() {
		return scope, nil, errors.New("deployment requires organization membership")
	}
	target, ok := s.targets[req.Target]
	if !ok || target.OrgID != scope.OrgID || target.RepoID != req.RepoID {
		return scope, nil, errors.New("no deployment target in this repository and organization")
	}
	run, err := s.resolveRun(ctx, req.RunID)
	if err != nil {
		return scope, nil, fmt.Errorf("resolve deployment run: %w", err)
	}
	if run.ID != req.RunID || run.OrgID != scope.OrgID || run.RepoID != req.RepoID || run.ActorID != scope.ActorID || run.ActorKind != scope.ActorKind {
		return scope, nil, errors.New("deployment run does not belong to this actor and repository")
	}
	...
	if scope.ActorKind == "agent" {
		...
		if grant.OrgID != scope.OrgID || grant.SubjectID != scope.ActorID || grant.SubjectKind != scope.ActorKind || !grant.ExpiresAt.After(time.Now()) {
			return scope, nil, errors.New("deployment grant missing, expired or outside actor scope")
		}
	}
	...
	if decision != approvals.DecisionHuman {
		return scope, nil, errors.New("deployment forbidden by policy")
	}
...
func (s *Service) Request(ctx context.Context, req Request) (Operation, error) {
	...
	scope, target, err := s.authorize(ctx, req)
	if err != nil {
...
func binding(op Operation) string {
	...
	}{op.Request, op.OrgID, op.ActorID, op.ActorKind, op.Environment, op.TargetRevision, op.Destination})
	return string(b)
}
...
func (s *Service) execute(ctx context.Context, id uuid.UUID, retry, observe bool) (Operation, error) {
	...
	if administrativeRecovery {
		...
		scope, target, err = s.authorize(ctx, op.Request)
		if err != nil {
			return Operation{}, err
		}
