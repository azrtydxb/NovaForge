...
// BeginWorkspace records the obligation BEFORE Kubernetes receives a create.
// An ambiguous create with no UID receipt remains pending, never absent-as-safe.
func (s *Store) BeginWorkspace(ctx context.Context, id uuid.UUID) error {
	scope, err := authz.FromContext(ctx)
	if err != nil {
		return err
	}
	tag, err := s.pool.Exec(ctx, `UPDATE agents.agent_runs SET workspace_cleanup_pending=true WHERE id=$1 AND org_id=$2 AND state='running' AND NOT execution_finished`, id, scope.OrgID)
	if err != nil {
		return err
	}
	if tag.RowsAffected() != 1 {
		return fmt.Errorf("run no longer owns workspace admission")
	}
	return nil
}

// AdoptWorkspaceCleanup records the exact resources discovered after a crash
// between provisioning and receipt persistence. It cannot replace any saved UID.
func (s *Store) AdoptWorkspaceCleanup(ctx context.Context, id workspace.Identity) error {
	if err := authz.RequireOrg(ctx, id.OrgID); err != nil {
		return err
	}
	if id.NamespaceUID == "" || id.PodUID == "" {
		return fmt.Errorf("workspace identity unavailable")
	}
	body, err := json.Marshal(id)
	if err != nil {
	...
	tag, err := s.pool.Exec(ctx, `UPDATE agents.agent_runs SET workspace_identity=$3 WHERE id=$1 AND org_id=$2 AND workspace_cleanup_pending AND workspace_identity IS NULL AND state IN ('failed','cancelled','over_budget','succeeded')`, id.RunID, id.OrgID, body)
	if err != nil {
		return err
	}
	if tag.RowsAffected() != 1 {
		return fmt.Errorf("workspace adoption no longer owns cleanup")
	}
	return nil
}
