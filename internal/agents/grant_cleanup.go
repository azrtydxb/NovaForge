...
func (s *Store) cleanupRunGrant(ctx context.Context, id, claim uuid.UUID) error {
   ...
   AND (grant_cleanup_pending OR work_release_pending OR workspace_cleanup_pending) AND grant_cleanup_lease_until<=clock_timestamp()`, claim, id, scope.OrgID)
	...
	if run.WorkspaceCleanupPending {
		if run.WorkspaceIdentity == nil && s.WorkspaceRecoverer != nil {
			identity, e := s.WorkspaceRecoverer(ctx, run.ID)
			if e == nil {
				e = s.AdoptWorkspaceCleanup(ctx, identity)
			}
			if e == nil {
				run.WorkspaceIdentity = &identity
			}
		}
		if s.WorkspaceCleaner == nil || run.WorkspaceIdentity == nil {
			...
			workspaceErr = s.ConfirmWorkspaceCleanup(workspaceCtx, run.ID)
			stop()
		}
	}
	...
	if run.WorkReleasePending {
		var pending bool
		auditErr := s.pool.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM agents.tool_calls WHERE org_id=$1 AND run_id=$2 AND outcome='pending')`, scope.OrgID, id).Scan(&pending)
		switch {
		case auditErr != nil || pending:
			releaseErr = fmt.Errorf("tool evidence reconciliation pending")
		...
		default:
			...
			releaseErr = s.WorkClaims.ReleaseExecution(ctx, ExecutionRelease{ExecutionClaim: run.ExecutionClaim(), Outcome: outcome, NoExecutionStarted: run.StartedAt.IsZero()})
		}
	}
