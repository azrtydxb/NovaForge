...
type ExecutionClaim struct{ WorkItemID, RepoID, RunID, AgentID, SponsorID uuid.UUID }
type ExecutionRelease struct {
	ExecutionClaim
	Outcome            string
	NoExecutionStarted bool
}
...
func (r Run) ExecutionClaim() ExecutionClaim {
	return ExecutionClaim{WorkItemID: r.WorkItemID, RepoID: r.RepoID, RunID: r.ID, AgentID: r.AgentID, SponsorID: r.SponsorID}
}
