...
func TestIssuanceRecoveryAndLateReplyAreFenced(t *testing.T) {
	for _, issueFirst := range []bool{false, true} {
		t.Run(fmt.Sprint(issueFirst), func(t *testing.T) {
			...
			// Recovery deliberately scans all organizations. Give this fixture its
			// own database rather than settling another test's live obligations
			// through this test's in-process Work owner.
			store := agents.NewStore(repositoryMetadataPool(t))
			store.WorkClaims = &workClaimsFixture{item: item}
			srv := agents.NewGRPCServer(store, ownerFixture{capability.NewStore(grantsPool(t))}, agentsRedis(t), &stubWorkClient{item: item}, nil)
			ctx := scopedCtx(org)
			agent := mustCreateAgent(t, store, ctx, org)
