...
// TestLocalExecutionRefusedByDefault pins the security posture: a CI job runs
// whatever the repository's workflow file says, so without an explicit opt-in
// the runner must refuse to run it on the host rather than doing so silently.
func TestLocalExecutionRefusedByDefault(t *testing.T) {
	t.Setenv("NOVAFORGE_ALLOW_LOCAL_EXEC", "")
	logs := make(chan string, 4)
	_, err := runner.Execute(context.Background(),
		&civ1.ConnectResponse{RunCmd: "echo pwned"}, t.TempDir(), logs)
	if !errors.Is(err, runner.ErrLocalExecutionNotPermitted) {
		t.Fatalf("want ErrLocalExecutionNotPermitted, got %v", err)
	}
}
