...
func dynamicBroker(t *testing.T, ttl time.Duration) (*secrets.Broker, context.Context, uuid.UUID, *baoFixture) {
	...
	org := uuid.New()
	ctx := scopedCtx(org)
	f := &baoFixture{duration: 1, leases: map[string]bool{}, leaseExpiry: map[string]time.Time{}}
	f.server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasPrefix(r.URL.Path, "/v1/database/creds/") {
			...
			if f.static {
				...
				f.leases[id] = true
				// The lifetime begins at issuance, not fixture construction:
				// database setup and negative auth probes issue no credential.
				// An explicit expiry remains available for contract violations.
				expiry := f.expires
				if expiry.IsZero() {
					expiry = time.Now().Add(ttl)
				}
				f.leaseExpiry[id] = expiry.Truncate(time.Microsecond)
			}
		...
		case "/v1/sys/leases/lookup":
			...
			json.NewEncoder(w).Encode(map[string]any{"data": map[string]any{"id": body.ID, "expire_time": f.leaseExpiry[body.ID]}})
