...
func TestLeaseRejectsValueSubstitution(t *testing.T) {
	// Issuance begins after the caller's minute-long authority budget starts;
	// the provider must return a shorter lifetime, not extend that budget.
	b, ctx, org, _ := dynamicBroker(t, 30*time.Second)
	run := uuid.New()
...
func TestConcurrentRedeemRevokeSerializes(t *testing.T) {
	// Leave room inside the caller's minute-long authority for issuance.
	b, ctx, org, _ := dynamicBroker(t, 30*time.Second)
