...
func TestReviewRevisionAndSummarySurviveRPCAndChangedHeadCannotMerge(t *testing.T) {
	...
	if git.request.GetExpectedSourceSha() != reviewedSHA || git.request.GetExpectedTargetSha() != reviewedSHA {
...
func TestAgentReviewInspectsDiffAndKeepsPreModelRevision(t *testing.T) {
	...
	if _, err := r.ReviewRun(proofServiceContext(t, ctx, "work-reviews"), run.ID, []string{"reviewer"}); err != nil {
		t.Fatal(err)
	}
...
func TestExceptionsRequireCurrentEligibleReview(t *testing.T) {
	...
	if err := store.RecordProof(proofServiceContext(t, ctx, "gates"), run.ID, "tests", "pass", "passed"); err != nil {
		t.Fatal(err)
	}
	reviewer := uuid.New()
