...
func TestGUIAuthenticatedAgentEvidenceAndEvents(t *testing.T) {
	...
	if err := audit.Complete(scope, callID, "ok", ""); err != nil {
		t.Fatal(err)
	}
	...
	// Redis is only a notification hint, not event authority. A live update
	// must originate in the runtime owner's durable audit ledger.
	probe, err := audit.Record(scope, agents.Entry{RunID: uuid.MustParse(run.GetId()), Tool: "gui.live.probe", ArgsJSON: []byte(`{}`)})
	if err != nil {
		t.Fatal(err)
	}
	if err := audit.Complete(scope, probe, "ok", ""); err != nil {
		t.Fatal(err)
	}
